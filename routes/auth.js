...
router.post(
  ...
  validationLoggin(),
  async (req, res, next) => {
    const { picture, username, email, password, location, birthDate, bio } = req.body;
    ...
    try {
      ...
      if (usernameExists) return next(createError(400));
        ...
        const newUser = await User.create({ picture, username, email, password: hashPass, location, birthDate, bio});
